...
func load_packs():
	...
	for pack in Data.ShopData:
		...
		var xpos = 7 + 265 * floor(index / 2)
		var ypos = index % 2 * 117 + 7
		
		new_shop_panel.pack_id = pack
		new_shop_panel.price = pack_info.PRICE
		new_shop_panel.position = Vector2(xpos, ypos)
		...
		new_shop_panel.size = Vector2(259, 110)
		
		$ShopPanel/Container.add_child(new_shop_panel)
		panels[new_shop_panel] = new_shop_panel.position
		index += 1
